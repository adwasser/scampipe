...
;--Run L.A. Cosmic on the flat-fielded images to remove cosmic-rays.
	...
	for i=0,(nFiles - 1) do begin
		...
		if (n_elements(bPix) gt 0) then begin
			...
			notSatPix = where(img[bPix] lt saturate*self.nonLinFrac, nNotSatPix, complement=satPix, nComplement=nSatPix);find CR pixels that are not saturated
...
;--Reproject Images to a Common Astrometric Solution			
	...
	if (nthreads gt 1) then begin
		...
		spawn, 'pwd', basedir
		for i=0,(nthreads - 1) do begin														;for each thread

...
FUNCTION fluxScaleLR, data0, scale, plot=plot
	...
	if (n_elements(plot) gt 0) then begin
		plot, dum0, yrange=[-15,15]+mean([median(dum0),median(dum1)]), title=strtrim(1d - res,2)
		oplot, dum1, color=fsc_color('red')
		pause
	endif
	RETURN, 1d - res
END
...
PRO chip2chipqe_fast, files, pre	
	...
	print, "stopping to switchover chip names"
	stop
...
;		'w67c1', 'w6c1', 'w7c3', 'w93c2', 'w9c2'] ;names of each detector
	...
	FOR i=0,(numExps - 1) DO BEGIN		;cycle through each exposure
		...
		print, "here1"	
		addit[*,i] = tnmin('newtify', x, bestmin=bestmin, /autoderivative)					;determine the relative scaling between chips
	...
	endif else finaddit = rebin(addit, [numchips, numexps])				;for newer data just take the median
	; Fix this

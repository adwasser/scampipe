...
PRO chip2chipqe_fast, files, pre, upgrade
	...
	file = strarr(numChips, numExps)
        if upgrade gt 0 then begin
           chip = ['chihiro', 'clarisse', 'fio', 'kiki', 'nausicaa', 'ponyo', 'san', 'satsuki', 'sheeta', 'sophie'] ;names of each detector
        endif else chip = ['w67c1', 'w6c1', 'si006s', 'si002s', 'w7c3', 'w93c2', 'w9c2', 'si005s', 'si001s', 'w4c5']
	...
	FOR i=0,(numExps - 1) DO BEGIN																	;cycle through each exposure
		...
		ENDIF ELSE BEGIN																			;otherwise
			if (date le 2008.58) then begin			;for older data with lingering non-linearity issues, allow a different scaling for different exposure times
				x = float([1.66667, 0.975279, 0.940946, 0.975610, 0.954359, 1.06361, 0.936070, 0.940543, 0.963282, 1.00205] * 0.6)
			endif else x = float([1.0, 0.962169, 0.933242, 0.970565, 1.01779, 1.09830, 0.960591, 0.939223, 0.979983, 1.03439])	;initial guesses at the scale, use last images values IF possible
		ENDELSE
...
;		statusbar, i, numExps, msg='Finding relative scaling between chips ...'
	...
	endif else if size(addit,/n_dimensions) then begin
        finaddit = addit
...
;		xticks=6, yrange=yrange, ystyle=1, xtickname=[' ','chi','cla','fio','kik','nau',' ']
 ;       if size(addit,/n_dimensions) lt 2 then begin
  ;;         oploterror, indgen(5), finaddit[0:4,0], (stddev(addit - finaddit)/sqrt(n_elements(addit[0,*])))[0:4], $
...
;        if size(addit,/n_dimensions) lt 2 then begin
;	ploterror, indgen(5), finaddit[5:*,0], (stddev(addit - finaddit)/sqrt(n_elements(addit[0,*])))[5:*], $
...
;	psclose
         ...
         if size(addit,/n_dimensions) lt 2 then begin
	mwrfits, arr_struct({name:chip, scale:finaddit[*,0], err:stddev(addit - finaddit)/sqrt(n_elements(addit))}), 'diag_supcamchipscale.fits', /create
            endif else $

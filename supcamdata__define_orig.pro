	...
	foreach flatType, flatTypes do begin	;make each flatfield type
		...
		if (strupcase(remake) eq 'YES') then begin
						...
						for ia=0,3 do begin
							...
							for iy=0,(nvert - 1) do begin
								...
								for ix=0,(nHorz - 1) do begin
									...
									++k
								endfor ; x axis of subchip
								undefine, subImg_y
							endfor ; y axis of subchip
							undefine, subImg
						endfor ; subchip 
						undefine, img
					endfor ; chips
			...
			for ic=0,(nchips - 1) do begin
				...
				; loop through exposure (?????????? why in different order than calc median?)
				for ixp=0,(nexp - 1) do begin
					img = mrdfits(fchip[ixp],0,hdr,/silent)
					vert = round(findgen(nvert + 1)/nvert*ysize)
					; loop through subimage
					for ia=0,3 do begin
						subImg = img[xmin[ia]:xmax[ia],*]
						; y axis of subimage
						for iy=0,(nvert - 1) do begin
							...
							for ix=0,(nHorz - 1) do begin
								...
								++k
							endfor ; x axis of subimage
							subImg[0,vert[iy]] = subImg_y
							undefine, subImg_y
						endfor ; y axis of subimage

...
class SuprimeCamMCMC(object):
    ...
    def get_wcs(self,filename):
        ...
        try:
            angle = -1. * np.array([math.degrees(math.acos(hdu_list[0].header['cd1_1'] / hdu_list[0].header['cdelt1']))])
        except KeyError:
    ...
    def ln_prior(self,theta):
        ...
        if np.any(np.abs(np.array(crpix2) - np.array(crpix2_i)) > 1000):
            return -np.inf
        if np.any(np.abs(np.array(crota2) - np.array(crota2_i)) > 2.5):
            return -np.inf
    ...
    def donate_astrometry(self,filename,meds=None,pre='aS',clobber=False):
        ...
        hdu.writeto(self.image_dir+pre+filename,clobber=clobber)

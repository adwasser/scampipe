...
img0 = mrdfits(newImage,/silent)


img = img0
iBad = where( (img le -32768), nBad)
if (nBad gt 0) then img[iBad] = !values.f_nan
xcen = list0.x
ycen = list0.y
objrad = 5
skyrad = [15,25]

	d = {xin:list0.x, yin:list0.y}

	flux = djs_phot(xcen, ycen, objrad, skyrad, img, cbox=30, salg='median', flerr=flerr, skyval=skyval, peakval=peakval)
	
	d = struct_addtags(d, {x:xcen, y:ycen, flux:flux, flerr:flerr, skyval:skyval, peakval:peakval})
	d = arr_struct(d)
	
	
	
	iBad = where( ~finite(d.flux) or ~finite(d.flerr) or ~finite(skyval) or (d.peakval ge datamax[0]), nBad)
	print, nBad
	dd = d[iBad]

	plot, d.x, d.yin - d.y, psym=1, yrange=[-1,1]*30
	oplot, dd.x, dd.yin - dd.y, psym=1, color=fsc_color('red')
	...
	plot, list0.x, list0.y - ycen, psym=1,  yrange=[-1,1]*20


	++xCen		;change back to 1 indexing
	++yCen		;change back to 1 indexing


	openw, lun, 'before.reg', /get_lun
	printf, lun, [transpose(list0.x), transpose(list0.y)]
	free_lun, lun


	openw, lun, 'after.reg', /get_lun
	printf, lun, [transpose(xcen), transpose(ycen)]
	free_lun, lun

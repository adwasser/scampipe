...
PRO supCamData::mkDir, str
	...
	if (ndir eq 0) then file_mkdir, str
END
			...
			for c=1,nchiplet do begin
				...
				for i=0,(noverscan - 1) do begin
					...
					if (i eq 0) then reg = img[xomin:xomax, *] else begin	;isolate the x-overscan region
						...
						reg = transpose(reg)				;transpose this in order to use 
												;the same code as the x-overscan
					endelse
		
					mn = mean(reg, dimension=1)				;calculate the mean of each row
					mn = transpose(rebin(mn, n_elements(reg[0,*]),n_elements(reg[*,0])))	
												;create 2d image with 
												;appropriate mean value placed at each pixel
					sig = stddev(reg, dimension=1)				;calculate the standard deviation of each row
					sig = transpose(rebin(sig, n_elements(reg[0,*]),n_elements(reg[*,0])))		
												;create 2d image with appropriate 
												;sigma value placed at each pixel
					...
					sig = stddev(reg, dimension=1, /nan)/sqrt(num)		;calculate the standard deviation of the 
												;mean for each row
...
;--Make Flat Field Frames (create obj_mflat*.fits files)
	...
	;create a 'pre' directory or retrieve files from it
	rfiles = self->file_check(nFiles)
	
	;file_mkdir, 'dir'+pre
	;file_move, 'dir'+pre+'/'+pre+'*.fits', './'
	;rFiles = file_search(pre+'*.fits')	;make a list of all the gTo_RH* files
	
	; Always make DOMEFLATS. If 'SKYFLAT' is chosen make that and DOMEFLATS.
	; if SUPERSKYFLAT is chosen check for other flattypes	
	CASE self.flattype of
		'DOMEFLAT' : flattypes = 'DOMEFLAT' 
		'SKYFLAT'  : flattypes = ['DOMEFLAT', 'SKYFLAT'] 
		'SUPERSKYFLAT' : 
			BEGIN
				flats = self->supCheckConstraints(rFiles, {FILTER01:self.filter, OBJECT:'SKYFLAT'})		
				if (n_elements(flats) eq 0) then begin
					flats = self->supCheckConstraints(rFiles, {FILTER01:self.filter, OBJECT:'DOMEFLAT'})	
					if (n_elements(flats) eq 0) then stop else flattypes = 'DOMEFLAT'
				endif else flattypes = ['DOMEFLAT', 'SKYFLAT']
			END
	ENDCASE						

	; For each flatType selected check to if such flats already exist
	; if they do confirm remaking them	
	foreach flatType, flatTypes do begin	
		...
		if (strupcase(remake) eq 'YES') then begin
			...
			; Find the median value of each mosachip, chip, and the local 
			progress = 0l
			for exp=0,(nExp - 1) do begin
				index = 0l
				for chip=0,(nchips - 1) do begin
					img = mrdfits(flats[exp*nchips+chip], 0, header, /silent)
		
					xsize = sxpar(header, 'NAXIS1')
					ysize = sxpar(header, 'NAXIS2')
					; Make an array to hold all the pixels
					if (chip eq 0) then holdpix = fltarr(long(sxpar(header,'NAXIS1'))*ysize*nchips) + !values.f_nan			
						
					xMin = [sxpar(header, 'S_EFMN11'), sxpar(header, 'S_EFMN21'), sxpar(header, 'S_EFMN31'), sxpar(header, 'S_EFMN41')] - 1l	
					xMax = [sxpar(header, 'S_EFMX11'), sxpar(header, 'S_EFMX21'), sxpar(header, 'S_EFMX31'), sxpar(header, 'S_EFMX41')] - 1l	
					for limg=0,3 do begin		
						subImg = img[xmin[ia]:xmax[limg],*]	
						nsubImg = n_elements(subImg)*1l
						holdpix[index] = subImg[lindgen(nsubImg)]
						index = index + nsubImg
						vert = round(findgen(nvert + 1)/nvert*ysize)
						for suby=0,(nvert - 1) do begin
							subImg_y = subImg[*,vert[suby]:(vert[suby+1]-1l)]
							horz = round(findgen(nHorz + 1)/nHorz*n_elements(subImg_y[*,0]))
							for subx=0,(nHorz - 1) do begin
								subImg_x = subImg_y[horz[subx]:(horz[subx+1]-1l),*]
								subImg_x = subImg_x[where(finite(subImg_x) and (subImg_x gt 500) and (subImg_x lt 60000), count)]
								locMed[chip, exp, limg, suby, subx] = median(subImg_x, /even)
								sigL[chip, exp, limg, suby, subx] = robust_sigma(subImg_x[where(subImg_x lt locMed[chip, exp, limg, suby, subx])] - locMed[chip, exp, limg, suby, subx], /zero)
								sigH[chip, exp, limg, suby, subx] = robust_sigma(subImg_x[where(subImg_x ge locMed[chip, exp, limg, suby, subx])] - locMed[chip, exp, limg, suby, subx], /zero)								
								undefine, subImg_x
								statusbar, progress, nChips*nExp*4*nVert*nHorz, msg='Calculating noise characteristics for flat-field frames ...'
								++progress
							endfor
							undefine, subImg_y
						endfor
				...
				medVals[exp] = median(holdPix,/even)
				undefine, holdpix
			endfor
			...
			for ic=0,(nchips - 1) do begin
					...
					for ia=0,3 do begin
						...
						for iy=0,(nvert - 1) do begin
							...
							for ix=0,(nHorz - 1) do begin
								print, locMed[ic,ixp,ia,iy,ix]
								subImg_x = subImg_y[horz[ix]:(horz[ix+1]-1l),*]
								bPix = where((subImg_x lt (locMed[ic,ixp,ia,iy,ix] - nsig[0]*sigL[ic,ixp,ia,iy,ix])) or $
									      (subImg_x ge (locMed[ic,ixp,ia,iy,ix] + nsig[1]*sigH[ic,ixp,ia,iy,ix])), nbad)
				...
				;spawn, 'mask_for_AGX '+outfile+' '+outfile+' '+strtrim(j_limit,2)+' -32768'
			endfor
		endif
	endforeach	;will end with flattype = SKYFLAT if it exists, otherwise it will be DOMEFLAT

	; move all the To_RH* files to a dir
	self->putInDir, self.pre+'*.fits', 'dir'+self.pre		

	; if SUPERSKYFLATs are specified make the final version
	; and put them in their own folders	
	if (strupcase(self.flatType) eq 'SUPERSKYFLAT') then begin	
		stop
		;i'm not sure the code below is correct..., should flatType be self.flatType?
		;supFinalSuperFlats, self.filter, flatType, sm=sm					
		spawn, 'mkdir ' + self.flatType + '_' + self.filter					
		spawn, 'mv obj_mflat*.fits ' + self.flatType + '_' + self.filter + '/'					
	endif
	...
	; move all the To_RH* files to a dir
	foreach file, 'diag_'+['sigL','sigH','medVals','locMed']+'.fits' do self->putInDir, file, 'dir_DIAG'
...
;--Multiplicatively scale chips (change from gqfTo_RH* to AgqfTo_RH*)
	...
	if (nFiles gt 0) then chip2chipqe_fast, files, post else stop
	self->putInDir, self.pre+'*.fits.gz', 'dir'+self.pre										;move all the To_RH* files to a dir
	self->gZip, post+'*.fits'
	self->putInDir, post+'*.fits.gz', 'dir'+post												;move all the To_RH* files to a dir
...
;--Distortion/Atmospheric Dispersion Correction (change qfTo_RH* to gqfTo_RH*)
	...
	print, "Due to path issues in another window run 'distcorr.csh distCorrList' and restart pipeline'
stop	
...
;--Masking the AG Probe (change from gqfTo_RH* to AgqfTo_RH*)
	...
	if flag then begin
		...
		self->putInDir, post+'*.fits.gz', 'dir'+post	
		self.pre = post	
	endif else self->fileDelete, post+'*.fits'
END
...
;--Calibrate astrometry
	...
	self->calibAstrom, files, post, self.date, redoastrom=redoastrom, catalog=catalog, $
		nthreads=nthreads, nobatch=nobatch, noinherit=noinherit, distcorr=distcorr			;perform astrometry, maybe better if I break this up so that things can be fixed more easily
...
PRO supCamData::calibAstrom, file, post, date, redoastrom=redo, catalog=catalog, nthreads=nthreads, nobatch=nobatch, noinherit=noinherit, distcorr=distcorr
	...
	if (n_elements(noinherit) eq 0) then begin
		for i=0,(n_elements(mosfile) - 1) do begin
			;print, 'SKIPPING FIRST MOSAIC FOR TESTING.....FIX THIS!!!!!!!'
			;print, 'SKIPPING FIRST MOSAIC FOR TESTING.....FIX THIS!!!!!!!'
			;print, 'SKIPPING FIRST MOSAIC FOR TESTING.....FIX THIS!!!!!!!'
			;print, 'SKIPPING FIRST MOSAIC FOR TESTING.....FIX THIS!!!!!!!'
...
;--Compare the flux levels of stars between exposures to determine the relative scaling
	...
	if (nMosFiles gt 1) then begin
		...
		if (nMatch gt 0) then begin																;if scaleFits already exists
			redo = 'NO'																			;then we don't need to remake it
			scFits = mrdfits(scaleFits,1,/silent)												;read it in
...
;--Reproject Images to a Common Astrometric Solution			
	...
	spawn, '/u/zach/montage/Montage_v3.3/bin/mImgtbl scratch_proj images-rawdir.tbl'											;Generate image metadata table
	spawn, '/u/zach/montage/Montage_v3.3/bin/mMakeHdr -n images-rawdir.tbl template.hdr'										;Make template header
	...
	if (nthreads gt 1) then begin
		...
		undefine, bridge																	;get rid of the bridge structure								
            endif ;else spawn, '~/montage/Montage_v3.3/bin/mProjExec -p scratch_proj images-rawdir.tbl template.hdr projdir stats.tbl' ;reproject each image
;       	endif      else spawn, 'u/zach/montage/Montage_v3.3/bin/mProjExec -p scratch_proj images-rawdir.tbl template.hdr projdir stats.tbl' ;reproject each image
        print,'Due to montage path issues, run: '
        print,'/u/zach/montage/Montage_v3.3/bin/mProjExec -p scratch_proj images-rawdir.tbl template.hdr projdir stats.tbl'
	...
	stop
	spawn, '/u/zach/montage/Montage_v3.3/bin/mImgtbl -c projdir projected.tbl'												;Generate new image metadata table
	...
	for i=0,(count - 1) do begin
		...
		if (nMatch gt 0) then begin
 		...
 		if sxpar(headfits(afiles[i]),'BITPIX') eq -64 then spawn, '/u/zach/montage/Montage_v3.3/bin/mConvert -b -32 ' + afiles[i] + ' ' + afiles[i]	;convert the file back to float
		self->gZip, aFiles[i]
	...
	for i=0,(nfiles - 1) do begin
		if (sxpar(headfits(files[i]),'BITPIX') eq -64) then spawn, '/u/zach/montage/Montage_v3.3/bin/mConvert -b -32 ' + files[i] + ' ' + files[i]		;convert the file back to float
...
PRO supCamData::coAdd
	...
	spawn, '/u/zach/montage/Montage_v3.3/bin/mAdd -p projdir -a ' + strlowcase(coaddInfo.coaddTyp) + ' projected.tbl template.hdr final/mosaic.fits'

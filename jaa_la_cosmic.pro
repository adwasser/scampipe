...
; REVISION HISTORY:
;   20-May-2001  Written by Joshua Bloom, Caltech (jsb@astro.caltech.edu)
;	28-Oct-2012  Added quiet keyword to suppress notifications
;				 -Jacob Arnold, UC Santa Cruz (jacob@ucolick.org)
;-
;------------------------------------------------------------------------------
function jaa_la_cosmic, imlist, outlist=outlist, masklist=masklist, sigclip=sigclip, $
               gain=gain, $
               readn=readn, $
...
;        SAME but slower: med5 = lacos_replace(med5,0.0001,'INDEF', 0.0)
        ...
        if ~n_elements(quiet) then print, 'Found ' + strtrim(string(ncosmicray)) + ' cosmic rays in iteration ' + strtrim(string(iter))
    ...
    if (masklist[i] eq imlist[i]) then begin
        ...
        maskname = tmp[0] + '.fits'
    endelse
    if ~n_elements(quiet) then begin
	    print, 'Writing mask   image = ' + maskname
	    print, '        output image = ' + outname
	endif
		
	bPix = where(outMask,nBpix)
	if (nBpix eq 0) then stop
	if 0 then begin
		grow = 1
		newIm = maskgrow(oldoutput, bPix, grow, !values.f_nan)
	endif else begin
		newIm = oldoutput
		newIm[bPix] = !values.f_nan
	endelse

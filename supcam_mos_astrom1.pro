...
function supcam_mos_astrom_getcrpix, arg1, res=res
	COMPILE_OPT idl2, HIDDEN 
	common sharemos1, dum, crval1, crval2
	arg = arg1
	crpix1 = arg[0]
	crpix2 = arg[1]
	cdelt1 = arg[2]
	...
	weights = dum.x*0 + 1d
	dstart = [-1*crpix1, -1*crpix2, cdelt1, cdelt2, crota2]
	quiet = 1
	res0 = xyrot_off(dum.x, dum.y, xi, eta, error, start=dstart, fixed=[0,0,1,1,0], yfit=yfit, weights=weights, quiet=quiet, /newt)		;rotation and magnificaion coefficients
...
;	print, resid
	...
	new = arr_struct({xra:newRA, xdec:newDec})
	return, new
...
;--install these into the catalog
	COMPILE_OPT idl2, HIDDEN 
	common sharemos, data0, gind, crpix1, crpix2, cdelt1, cdelt2, crota2, counter, flag, results
	common sharemos1
	...
	for i=0,(n_tags(data0)-1) do begin
		dum = data0.(i)
		arg1 = [crpix1[i], crpix2[i], cdelt1[i], cdelt2[i], crota2[i]]
		new = supcam_mos_astrom_getcrpix(arg1, res=res)
		new = struct_addtags(new, replicate({chip:i},n_elements(new)))
		dum = struct_addtags(dum, new)
...
function supcam_mos_astrom_newtify, arg
	...
	result = sqrt( robust_sigma(cat.ra - cat.xra, /zero)^2d + robust_sigma(cat.dec - cat.xdec, /zero)^2d )
	...
	if 0 then begin
		...
		ndec = 2
		minra = min(cat.ra)
		rainc = (max(cat.ra) - minra)/double(nra) + 1d-5
		mindec = min(cat.dec)
		decinc = (max(cat.dec) - mindec)/double(ndec) + 1d-5
		for i=0,(nra - 1) do begin
			for j=0,(ndec - 1) do begin
				ind = where((cat.ra ge (minra + rainc*i)) and (cat.ra lt (minra + rainc*(i+1))) and (cat.dec ge (mindec + decinc*j)) and (cat.dec lt (mindec + decinc*(j+1))))
				result = result + sqrt( robust_sigma(cat[ind].ra - cat[ind].xra, /zero)^2d + robust_sigma(cat[ind].dec - cat[ind].xdec, /zero)^2d )
			endfor
		endfor
	endif
	
	;result = total(abs([cat.ra  - cat.xra, cat.dec - cat.xdec]))
	...
	if ((counter mod 1) eq 0) then begin
		...
		window, 3
		cleanplot, /silent
		plot, cat.ra, cat.dec, psym=1, /iso, xrange=reverse(minmax(cat.ra)), yrange=minmax(cat.dec), xstyle=1, ystyle=1
		oplot, cat.xra, cat.xdec, psym=1, color=fsc_color('red')
	
		window, 2
		cleanplot, /silent
		plot, [results]*3600d, psym=-1, /ylog
		wset, 0
	endif
	;print, [arg, result]
	return, result
...
pro supcam_mos_astrom_plot_resid, cat, ps=ps, outfile=outfile
	...
	yrange = [-1,1]*0.6
	multiplot
	plot, cat.ra, (cat.ra - cat.xra)*3600d, xrange=reverse(minmax(cat.ra)), yrange=yrange, xstyle=1, ystyle=1, psym=8, ytitle=textoidl('\Delta RA'), ytitle=textoidl('\alpha_{RMS} = ')+roundx(robust_sigma(cat.ra - cat.xra)*3600d,3)+'"'
	oplot, (findgen(10)-5)*1000, intarr(10), linestyle=2, color=fsc_Color('red')
	sharpcorners, thick=3
	multiplot
	plot, cat.dec, (cat.ra - cat.xra)*3600d, xrange=minmax(cat.dec), yrange=yrange, xstyle=1, ystyle=1, psym=8, ytitle=textoidl('\delta_{RMS} = ')+roundx(robust_sigma(cat.dec - cat.xdec)*3600d,3)+'"'
	oplot, (findgen(10)-5)*1000, intarr(10), linestyle=2, color=fsc_Color('red')
	sharpcorners, thick=3
	multiplot
...
pro supcam_mos_astrom_writeastrom, filenames, nfiles, astr, post=post
	...
	for i=0,(nfiles-1) do begin
		...
		sxaddpar, hdr, 'CDELT1', astr.cdelt1[i], /saveComment
		sxaddpar, hdr, 'CDELT2', astr.cdelt2[i], /saveComment
...
function supcam_mos_astrom1, data, date, filenames=filenames, maxiter=maxiter, cat=cat, start=start, distcorr=distcorr, post=post
	COMPILE_OPT idl2, HIDDEN 
	...
	if (n_elements(start) eq 0) then begin
		CDELT1 = fltarr(10) - 5.6093104d-05							;initial guess at RA scale (deg/pixel)
		CDELT2 = fltarr(10) + 5.6095829d-05
